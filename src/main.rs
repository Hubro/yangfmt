...
use crate::parsing::{parse, BlockNode, LeafNode, Node, NodeHelpers, RootNode};
...
fn write_node<T: std::io::Write>(
    ...
    macro_rules! write_keyword {
        ($keyword:expr) => {
            match $keyword {
                parsing::StatementKeyword::Keyword(text) => write!(out, "{text}")?,
                parsing::StatementKeyword::ExtensionKeyword(text) => write!(out, "{text}")?,
                parsing::StatementKeyword::Invalid(text) => write!(out, "{text}")?,
            };
        };
    }
    ...
    macro_rules! write_value {
        ($keyword:expr, $value:expr) => {
            match $value {
                parsing::NodeValue::Date(text) => write!(out, "{text}")?,
                parsing::NodeValue::Number(text) => write!(out, "{text}")?,
                parsing::NodeValue::String(text) => write!(out, "{text}")?,
                parsing::NodeValue::Other(text) => write!(out, "{text}")?,
                parsing::NodeValue::StringConcatenation(strings) => {
